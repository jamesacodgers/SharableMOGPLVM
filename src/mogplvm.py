# Class containing the MOGPLVM logic
...
class MOGPLVM(nn.Module):
    def __init__(
        ...
        # TODO: play with different hyperparamters for each component
    ):
        torch.set_default_dtype(torch.float64)
        super(MOGPLVM, self).__init__()
        self._log_beta = nn.Parameter(torch.log(beta))
        if gamma is not None:
            self._log_gamma = nn.Parameter(torch.log(gamma))
        else:
            assert v_l is None
            self._log_gamma = None
        self._log_sigma2 = nn.Parameter(torch.log(sigma2))

        self._log_sigma2_s = nn.Parameter(torch.log(sigma2_s))

        self.v_x = nn.Parameter(v_x)
        if v_l is not None:
            self.v_l = nn.Parameter(v_l)
        else:
    ...
    def elbo(self, datasets: List[Dataset]) -> torch.Tensor:
        ...
        x_kl_divergence = torch.zeros(1)
        r_prior_term = torch.zeros(1)
        u_independent_terms = torch.zeros(1)
        for dataset in datasets:
            ...
            u_independent_terms += (
                -dataset.observations.num_data_points
    ...
    def get_K_vv(self) -> torch.Tensor:
        ...
        if self.v_l is None:
            return self.sigma2_s * K_vxvx + 1e-6 * torch.eye(self.num_inducing_points)
        ...
        return self.sigma2_s * tile_final_dims_to_matrix(
            K_vv_expanded
        ) + 1e-6 * torch.eye(self.num_inducing_points)
    ...
    def get_xi_1(self, datasets: List[Dataset]) -> torch.Tensor:
        if self.v_l is None:
            xi_1 = torch.zeros(
                ...
                self.num_inducing_points * datasets[0].num_components,
            )
        else:
            xi_1 = torch.zeros(self.num_inducing_points * datasets[0].num_components)
        for dataset in datasets:
    ...
    def _get_xi_2_single(self, dataset: Dataset) -> torch.Tensor:
        ...
        # print("do expansion")
        # print("sum")
        helper_ones = torch.ones(xi_2_expanded.shape[0])
    ...
    def get_psi_2_x(self, mu_x: torch.Tensor, Sigma_x: torch.Tensor) -> torch.Tensor:
        ...
        helper_ones = torch.ones(self.num_latent_dims)
    ...
    def sigma2(self, value):
        self._log_sigma2 = torch.nn.Parameter(torch.log(torch.Tensor([value])))
