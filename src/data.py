...
from src.utils.prob_utils import KL_dirichlet_and_uniform_dirichlet, dirichlet_mean, dirichlet_cov, multinomial_cov, multinomial_kl, multinomial_mean
...
class ObservedComponents(AbstractComponents):
    ...
    #TODO: tests
    def get_prior_term(self) -> torch.Tensor:
        distribution = torch.distributions.Dirichlet(torch.ones(self.num_components))
        p = torch.zeros(1)
...
class VariationalDirichletDistribution(torch.nn.Module, AbstractComponents):
    def __init__(self, alpha: torch.tensor) -> None:
        ...
        self._log_prior_alpha = torch.zeros(alpha.shape[1])
...
class VariationalClassifier(torch.nn.Module, AbstractComponents):
    def __init__(self, p: torch.Tensor) -> None:
        ...
        self._log_prior_p = torch.zeros(p.shape[1])
...
# def get_categorial_latent_init(training_observations: AbstractObservations, training_components: ObservedComponents, num_latent_dimensions):
#     for r in training_components.get_r().T:
#         select_category = training_observations.get_observations[r]
#         mean = select_category.mean(axis = 0)
#         E = training_observations.get_observations()[r] - mean
#         _, _, Vh = torch.linalg.svd(E)
#         x_init = training_observations.get_observations()[r]@Vh.T
#         return x_init[:,:num_latent_dimensions]/torch.sqrt(x_init[:, :num_latent_dimensions].var(axis = 0))