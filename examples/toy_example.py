...
# training_components = torch.distributions.Dirichlet(torch.ones(2)).sample((num_training_samples,))
...
# training_spectra = IndependentObservations(training_observed_spectra)
...
# %%
# s = bass.get_sample_mean(training_dataset)
# with torch.no_grad():
#     plt.plot(s[0, :, 0])
#     plt.plot(s[0, :, 1])
#     plt.show()
...
# %%
adam_training = torch.optim.Adam(chain(bass.parameters(), training_dataset.parameters()), lr = 1e-2)

loss.extend(train_bass_on_spectral_data(bass, [training_dataset], adam_training, 2000))
...
with torch.no_grad():
    for i in range(3):
        ...
        mu_l = bass.get_point_mean(x[i].reshape(1,-1), [training_dataset])
        sigma_l = bass.get_point_var(x[i].reshape(1,-1), [training_dataset])
...
with torch.no_grad():
    ...
    # plt.figure()
    # plt.scatter(
    #     training_dataset.mu_x[:,args[0]], 
    #     training_dataset.Sigma_x[:,args[1]], 
    #     )
    # plt.vlines(bass.v[:,0], 0, 2, color = "black", alpha =0.05)
    # plt.vlines(bass.v[:,0].mean(), 0, 2, color = "black", alpha =1)
    # plt.vlines(-bass.v[:,0].mean(), 0, 2, color = "black", alpha =1,  )

    # plt.figure()
    # plt.errorbar(true_latent_points.flatten(), training_dataset.mu_x[:, 0], 2*torch.sqrt(training_dataset.Sigma_x[:, 0]), linestyle="None", capsize=2, marker= ".", alpha = 0.2)


    s = bass.get_sample_mean(training_dataset, [training_dataset])
    for i in range(1):
        fig, ax = plt.subplots()
...
print("beta", bass.beta)
print("gamma", bass.gamma)
print("sigma2", bass.sigma2)
print("sigma2_s", bass.sigma2_s)
# %%
...
# %%
with torch.no_grad():
    plt.plot(range(training_spectra.num_measurements_per_datapoint), training_spectra.get_observations().T, c = "blue", alpha = 0.3)
    # plt.plot(range(training_spectra.num_measurements_per_datapoint), test_spectra.spectra.T, c = "orange", alpha = 0.3)
    plt.plot(range(training_spectra.num_measurements_per_datapoint), test_spectra.get_observations().T, c = "orange", alpha = 0.3)
...
for s in noise_schedule:
    print(s)
    bass.sigma2 = s 
    test_loss.extend(lbfgs_training_loop(bass, [training_dataset, test_dataset], test_dataset.parameters(), 2))

test_loss.extend(lbfgs_training_loop(bass, 
                                     [training_dataset, test_dataset], 
                                     chain(test_dataset.parameters(), training_dataset.parameters(), bass.parameters()),
                                     20))
...
plt.figure()
for i in range(2):
    plt.scatter(test_components[:,i], y_hat[:,i])
plt.plot([0,1],[0,1])
...
with torch.no_grad():
    ...
    # plt.savefig(f"examples/figs/toy_example/estimated_vs_real_r_{i+1}.pdf", bbox_inches='tight')
    relevant_lv = torch.argmin(bass.beta)
    ...
    # plt.savefig("examples/figs/toy_example/estimated_vs_real.pdf", bbox_inches='tight')
...
with torch.no_grad():
    ...
    # plt.savefig("examples/figs/toy_example/training_data.pdf")
    ...
    # plt.savefig("examples/figs/toy_example/data.pdf", bbox_inches="tight")
    plt.show()
...
with torch.no_grad():
    ...
    for i in range(3):
        for j in range(2):
            ...
            sigma_l = bass.get_point_var(x[i].reshape(1,-1), [training_dataset])
            ground_truth = get_pure_components(torch.Tensor(true_vals[i]))
            # ax[i].plot(wavelengths, mu_l[0])
            # ax[i].plot(wavelengths, mu_l[1])
            # ax[i].plot(wavelengths, ground_truth[0,:,0])
            # ax[i].plot(wavelengths, ground_truth[0,:,1])
            # ax[i].fill_between(wavelengths, mu_l[0]+2*torch.sqrt(sigma_l[0]), mu_l[0]-2*torch.sqrt(sigma_l[0]), alpha = 0.5)
            # ax[i].fill_between(wavelengths, mu_l[1]+2*torch.sqrt(sigma_l[1]), mu_l[1]-2*torch.sqrt(sigma_l[1]), alpha = 0.5)
    ...
    x_minus_2_handle = mlines.Line2D([], [], color='blue', label='x = -2')
    x_zero_handle = mlines.Line2D([], [], color='orange', label='x = 0')
    x_two_handle = mlines.Line2D([], [], color='green', label='x = 2')
    ax[0].legend(handles=[true_function_handle, (mean_estimated_handle, ci_handle), x_minus_2_handle, x_zero_handle, x_two_handle], labels = ["True Signals", "GP Estimate", "x = -2", "x = 0", "x=2"])
    ax[1].set_xlabel("$\lambda$")
    ax[0].set_ylabel("$s_1(x, \lambda)$")
    ax[1].set_ylabel("$s_2(x, \lambda)$")
    fig.subplots_adjust(hspace = 0.1)
    # plt.savefig("examples/figs/toy_example/gp_mapping.pdf", bbox_inches="tight")
    plt.show()
